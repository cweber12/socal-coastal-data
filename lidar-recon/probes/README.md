...
| `scripps2005_header.py` | LAS 1.1 public header **and** VLR GeoKeys for all 43 `CA_SCRIPPS_APR_2005` tiles, in one 375-byte range read each. Reports the vertical GeoKeys as **absent** rather than defaulting to a datum, and records tile 000012's HTTP 404 rather than skipping it. Produced `findings/scripps2005-headers.json`. Added under #109. |
| `scripps2005_gpstime.py` | Acquisition time from the point records. The first point of each LAZ chunk is stored uncompressed, so GPS week time costs 28 bytes per tile and no laszip. Decodes to day-of-week and time-of-day; the calendar week comes from the FGDC `begdate`/`enddate`. Produced `findings/scripps2005-gpstime.json`. Added under #109. |
| `scripps2005_acquisition_tide.py` | CO-OPS **observed** water level during the flight window — `water_level`, not the predictions `tides.py` uses, because what the laser saw is the actual water and #104 measures predictions drifting off the datum epoch. Records all four candidate dates including the three that fail. Produced `findings/scripps2005-acquisition-tide.json`. Added under #109. |
| `scripps2005_cabrillo_tie.py` | The datum tie: the raw cloud against the 124 NPS laser-levelled points from #83. Carries its own stdlib UTM forward projection so this directory stays dependency-free. Needs a local CSV of the cloud, which is not in this repo and says so. Produced `findings/scripps2005-cabrillo-tie.json`. Added under #109. |
