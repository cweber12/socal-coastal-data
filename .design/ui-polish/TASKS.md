...
- `npm run test` (203 lib tests) and `npm run typecheck` stay green.
...
- [ ] **1. Apply the token layer.** Merge [DESIGN_TOKENS.css](DESIGN_TOKENS.css)
...
- [ ] **2. Floor gap in the cell.** Add an optional gap slot to `TideLine`,
...
- [ ] **3. Floor in the row header.** Show each spot's floor beside its name in
...
- [ ] **4. Make the sort control work.** Move `sortRows` out of `app/page.tsx`
...
- [ ] **6. Migrate to the type scale.** Replace the 79 arbitrary font-size
...
- [ ] **7. Give the popover a tail.** Add a visual connector from the flag
...
- [ ] **8. Restyle the tinted panels.** Constrain to `--panel-measure` so a
...
- [ ] **9. Stop the spot page naming itself twice.** `SpotHeader` suppresses its
...
- [ ] **10. Fix the day chart's typography and label collisions.** Two separate
...
- [ ] **11. Mobile pass at 375px.** The single visible column is the only place
...
- [ ] **12. Accessibility pass.** Specific checks, not a general sweep:
